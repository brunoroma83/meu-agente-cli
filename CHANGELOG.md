...
### Added
- **Configuração de Modelos e Provedores Externos (/models)**: Extensão do comando `/models` e do módulo `llm.py` para suportar provedores de grande porte (OpenAI, Google Gemini, Anthropic Claude, DeepSeek, Alibaba Qwen, Moonshot Kimi e Provedores Personalizados compatíveis com OpenAI) com suporte à persistência segura das chaves de API (`API KEY`) no banco de dados e roteamento nativo sem dependências SDK extras.
- **Prevenção de Hangs em Bots e Ajuste de Status**: O comando `/models` agora é gerenciado diretamente de forma não bloqueante no Telegram, e o comando `/status` testa e pinga o provedor de LLM atualmente ativo (seja local ou externo) realizando uma validação de credencial em tempo real de 1 token.
